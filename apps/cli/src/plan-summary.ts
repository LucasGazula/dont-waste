...
const reversalHints: Partial<Record<ToolId, string>> = {
  headroom: "dont-waste uninstall removes marker-owned MCP blocks; use rollback <id> for snapshots",
  ...
  ponytail: "dont-waste uninstall removes owned config/plugin entries; preserve user plugins",
};

function pathTouchesAgent(file: string, agent: AgentId): boolean {
  const lower = file.toLowerCase();
  const token = agent === "claude-code" ? "claude"
    : agent === "gemini-cli" ? "gemini"
      : agent === "copilot-cli" ? "copilot"
        : agent === "antigravity-cli" ? "antigravity"
          : agent;
...
export function summarizePlanByAgent(input: PlanSummaryInput): AgentPlanRow[] {
  if (!input.selectedAgents.length) {
    return [{
      ...
      compatibility: ["install-only profile: no agent integrations will be activated"],
      reversal: ["No agent configs written in install-only mode"],
      notes: ["Advanced controls (CCR/TTL/cavecrew/MCP-shrink) are not exposed in this TUI yet"],
    }];
  }

  return input.selectedAgents.map((agent) => {
    const tools = input.plans.map((plan) => plan.tool);
    const files = [...new Set(input.plans.flatMap((plan) => plan.affectedPaths.filter((file) => pathTouchesAgent(file, agent))))];
    const compatibility = input.plans.flatMap((plan) => {
      ...
      return [`${plan.tool}: ${capability.capability.installMethod} · metrics=${capability.capability.supportsMetrics}`];
    });
    const reversal = tools.map((tool) => reversalHints[tool]).filter((item): item is string => Boolean(item));
    const notes = [
      ...input.plans.flatMap((plan) => plan.warnings.filter((warning) => warning.toLowerCase().includes(agent)).slice(0, 1)),
      ...tools.map((tool) => restartHints[tool]).filter((item): item is string => Boolean(item)),
...
export function formatPlanSummary(input: PlanSummaryInput): string {
  ...
  const toolBlocks = input.plans.map((item) => {
    const commands = item.commands.map((command) => `  ${command.interactive ? "[interactive/launch-only] " : ""}${command.command} ${command.args.join(" ")}`).join("\n") || "  already installed / no command";
  ...
  const agentTable = rows.length
    ? rows.map((row) => {
      const files = row.files.length ? row.files.map((file) => `    - ${file}`).join("\n") : "    - (tool-global / none yet)";
      return [
        `${row.agent}`,
        `  tools: ${row.tools.join(", ") || "none"}`,
        `  restart: ${row.restartRequired ? "yes" : "no"}`,
        `  files:\n${files}`,
        row.compatibility.length ? `  compatibility: ${row.compatibility.join(" · ")}` : "",
        row.reversal.length ? `  reversal: ${row.reversal.join(" · ")}` : "",
