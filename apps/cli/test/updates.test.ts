...
import { compareUpdates, compareVersions, normalizeVersion, toolsNeedingUpdate } from "../src/updates.js";
  ...
  it("classifies tools that need an upgrade plan", () => {
    const comparisons = compareUpdates(
      ...
      [
        ...
        { tool: "headroom", latest: "0.21.0", url: "https://example.com/headroom" },
        { tool: "caveman", latest: "v2.0.0", url: "https://example.com/caveman" },
        { tool: "ponytail", url: "https://example.com/ponytail", error: "rate limited" },
      ],
    );
    expect(comparisons.find((item) => item.tool === "rtk")?.status).toBe("update-available");
    expect(comparisons.find((item) => item.tool === "headroom")?.status).toBe("up-to-date");
    expect(comparisons.find((item) => item.tool === "caveman")?.status).toBe("not-installed");
    expect(comparisons.find((item) => item.tool === "ponytail")?.status).toBe("error");
    expect(toolsNeedingUpdate(comparisons)).toEqual(["rtk", "caveman"]);
    expect(toolsNeedingUpdate([
      { tool: "ponytail", installed: undefined, latest: "v4.8.4", url: "x", status: "unknown" },
    ])).toEqual([]);
  });
});
