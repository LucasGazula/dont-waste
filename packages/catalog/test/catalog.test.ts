...
describe("compatibility catalog", () => {
  it("declares every supported agent for every orchestrated tool", () => {
    ...
    expect(getCapability("rtk", "codex")).toMatchObject({ installMethod: "hook", supportsMetrics: "measured" });
    expect(getCapability("ponytail", "opencode")).toMatchObject({ installMethod: "plugin", supportsMetrics: "unavailable" });
  });
});
