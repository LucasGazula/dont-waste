...
const sample: MetricEvent[] = [
  { id: "m1", occurredAt: "2026-07-06T10:00:00Z", tool: "rtk", metricType: "command-output", tokensBefore: 100, tokensAfter: 20, tokensSaved: 80, confidence: "measured", sourceDepth: 0, overlapKey: "flow-a", projectPath: "/tmp/a", sessionId: "s1", costBefore: 0.02, costAfter: 0.01 },
  { id: "m2", occurredAt: "2026-07-06T11:00:00Z", tool: "headroom", metricType: "context", tokensBefore: 50, tokensAfter: 40, tokensSaved: 10, confidence: "measured", sourceDepth: 1, overlapKey: "flow-a", projectPath: "/tmp/a", sessionId: "s1" },
  { id: "e1", occurredAt: "2026-07-08T10:00:00Z", tool: "caveman", metricType: "estimated-output", tokensBefore: 40, tokensAfter: 10, tokensSaved: 30, confidence: "estimated", sourceDepth: 0, projectPath: "/tmp/b", sessionId: "s2" },
  { id: "u1", occurredAt: "2026-07-08T12:00:00Z", tool: "ponytail", metricType: "output", tokensBefore: null, tokensAfter: null, tokensSaved: null, confidence: "unavailable", sourceDepth: 0 },
];
...
describe("event filters", () => {
  it("filters by confidence, tool, project fragment, and session", () => {
    ...
    expect(filterEvents(sample, { tool: "caveman" }).map((e) => e.id)).toEqual(["e1"]);
    expect(filterEvents(sample, { project: "/tmp/b" }).map((e) => e.id)).toEqual(["e1"]);
    expect(filterEvents(sample, { session: "s1" })).toHaveLength(2);
    expect(filterEvents(sample, { confidence: "unavailable", tool: "ponytail" })).toHaveLength(1);
  });

  it("reports overlap groups and cost totals", () => {
    expect(overlapGroups(sample)).toEqual([{ overlapKey: "flow-a", tools: ["rtk", "headroom"], depths: [0, 1] }]);
    expect(costTotals(sample)).toEqual({ costBefore: 0.02, costAfter: 0.01, saved: 0.01 });
  });
});
...
describe("dashboard overview contract", () => {
  it("exposes daily/weekly/costs/privacy/sessions without raw dumps", () => {
    ...
    expect(overview.privacy).toMatchObject({ storesPrompts: false, storesOutputs: false });
