...
import { installRtkFromOfficialRelease, resolveRtkTarget } from "./rtk-release.js";
...
const RTK_RELEASE_LABEL = "Install RTK from official GitHub release with SHA-256 verification";
...
export class RtkAdapter extends BaseAdapter {
  ...
  async planInstall(selection: ToolSelection, context: AdapterContext): Promise<OperationPlan> {
    ...
    if (!detected.detected) {
      if (context.platform === "darwin" && await findExecutable("brew", context.platform)) {
        commands.push({ command: "brew", args: ["install", "rtk"], label: "Install RTK with Homebrew" });
      } else {
  ...
  async install(plan: OperationPlan, context: AdapterContext): Promise<InstallResult> {
    ...
    for (const command of plan.commands) {
      if (command.label === RTK_RELEASE_LABEL) {
        ...
        try {
          const installed = await installRtkFromOfficialRelease({ platform: context.platform, dryRun: false });
          executed.push(command);
          if (!await findExecutable("rtk", context.platform)) {
            errors.push(`RTK installed to ${installed.binaryPath} but is not on PATH. Add ~/.local/bin to PATH and rerun.`);
            break;
          }
        } catch (error) {
          executed.push(command);
      ...
      if (result.ran) executed.push(command); else skipped.push(command);
      if (result.error) {
