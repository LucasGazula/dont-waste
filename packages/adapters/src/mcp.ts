...
export type McpRegisterStatus = "registered" | "already" | "mismatch" | "unsupported" | "failed";
...
function specsMatch(existing: McpServerSpec, requested: McpServerSpec): boolean {
  return existing.command === requested.command
...
function renderCodexBlock(spec: McpServerSpec): string {
  ...
  if (spec.args.length) lines.push(`args = [${spec.args.map(tomlString).join(", ")}]`);
  if (spec.env && Object.keys(spec.env).length) {
    lines.push("");
    lines.push(`[mcp_servers.${spec.name}.env]`);
    for (const [key, value] of Object.entries(spec.env)) lines.push(`${key} = ${tomlString(value)}`);
  }
  lines.push(MARKER_END);
  return lines.join("\n");
}

function parseCodexServer(content: string, name: string): McpServerSpec | undefined {
  const section = content.match(new RegExp(`\\[mcp_servers\\.${name}\\]([\\s\\S]*?)(?=\\n\\[|$)`));
  if (!section) return undefined;
  const body = section[1] ?? "";
  const command = body.match(/^\s*command\s*=\s*"((?:\\.|[^"])*)"/m)?.[1]?.replaceAll("\\\"", "\"").replaceAll("\\\\", "\\");
  if (!command) return undefined;
  const argsMatch = body.match(/^\s*args\s*=\s*\[([^\]]*)\]/m);
  const args = argsMatch?.[1]
    ? [...argsMatch[1].matchAll(/"((?:\\.|[^"])*)"/g)].map((item) => item[1]!.replaceAll("\\\"", "\"").replaceAll("\\\\", "\\"))
    : [];
...
export function claudeMcpConfigPath(context: Pick<AdapterContext, "home">): string {
  return path.join(context.home, ".claude", "mcp.json");
}

export function opencodeConfigPath(context: Pick<AdapterContext, "home" | "platform">): string {
  return path.join(context.home, ".config", "opencode", "opencode.json");
}

export function mcpConfigPath(agent: AgentId, context: Pick<AdapterContext, "home" | "platform">): string | undefined {
...
export async function readMcpServer(agent: AgentId, context: Pick<AdapterContext, "home" | "platform">, name: string): Promise<McpServerSpec | undefined> {
  ...
  try {
    ...
    if (agent === "claude-code") {
      ...
      return {
        ...
        env: record.env && typeof record.env === "object" && !Array.isArray(record.env)
          ? Object.fromEntries(Object.entries(record.env as Record<string, unknown>).filter((item): item is [string, string] => typeof item[1] === "string"))
          : undefined,
      };
    }
    const mcp = root.mcp;
...
async function registerCodex(spec: McpServerSpec, context: Pick<AdapterContext, "home">): Promise<McpRegisterResult> {
  const file = codexConfigPath(context);
  const existing = await readMcpServer("codex", { ...context, platform: "linux" }, spec.name);
  if (existing && specsMatch(existing, spec)) {
    return { agent: "codex", status: "already", path: file, detail: "matches current configuration" };
  }
  let content = "";
  ...
  if (existing && !content.includes(MARKER_START)) {
    return { agent: "codex", status: "mismatch", path: file, detail: "user-managed [mcp_servers.headroom] entry outside Headroom markers; left untouched" };
  }
  ...
  } else if (content.trim()) next = `${content.replace(/\n*$/, "")}\n\n${block}\n`;
  else next = `${block}\n`;
  await mkdir(path.dirname(file), { recursive: true });
  await writeFile(file, next, "utf8");
  return { agent: "codex", status: "registered", path: file, detail: `wrote ${file}` };
}

async function registerClaude(spec: McpServerSpec, context: Pick<AdapterContext, "home">): Promise<McpRegisterResult> {
  const file = claudeMcpConfigPath(context);
  const existing = await readMcpServer("claude-code", { ...context, platform: "linux" }, spec.name);
  if (existing && specsMatch(existing, spec)) {
    return { agent: "claude-code", status: "already", path: file, detail: "matches current configuration" };
  }
  if (existing) {
    return { agent: "claude-code", status: "mismatch", path: file, detail: "existing headroom MCP entry differs; left untouched" };
  }
  ...
  return { agent: "claude-code", status: "registered", path: file, detail: `wrote ${file}` };
}

async function registerOpencode(spec: McpServerSpec, context: Pick<AdapterContext, "home" | "platform">): Promise<McpRegisterResult> {
  const file = opencodeConfigPath(context);
  const existing = await readMcpServer("opencode", context, spec.name);
  if (existing && specsMatch(existing, spec)) {
    return { agent: "opencode", status: "already", path: file, detail: "matches current configuration" };
  }
  if (existing) {
    return { agent: "opencode", status: "mismatch", path: file, detail: "existing headroom MCP entry differs; left untouched" };
  }
  let current: Record<string, unknown> = {};
  try {
    const parsed: unknown = JSON.parse(await readFile(file, "utf8"));
  ...
  return { agent: "opencode", status: "registered", path: file, detail: `wrote ${file}` };
}

export async function registerHeadroomMcp(agent: AgentId, spec: McpServerSpec, context: Pick<AdapterContext, "home" | "platform">): Promise<McpRegisterResult> {
  try {
    ...
    return { agent, status: "unsupported", detail: `${agent} has no structured Headroom MCP registrar yet` };
  } catch (error) {
