...
describe("phase 0/1 security regressions", () => {
  it("does not treat Node presence as Caveman/Ponytail installed", async () => {
    ...
    const caveman = await new CavemanAdapter().detect({
      platform: "linux",
      home,
      selectedAgents: [],
      dryRun: true,
    });
    const ponytail = await new PonytailAdapter().detect({
      platform: "linux",
      home,
      selectedAgents: [],
      dryRun: true,
  ...
  it("detects Caveman/Ponytail from markers/config under a temp HOME", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "dont-waste-detect-yes-"),
    );
  ...
  it("install-only does not write Caveman or Ponytail global configs", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "dont-waste-install-only-"),
    );
    const ctx = {
      platform: "linux" as const,
      home,
      selectedAgents: [] as const,
      dryRun: false,
    };
    const caveman = new CavemanAdapter();
    const ponytail = new PonytailAdapter();
    const cavemanPlan = await caveman.planInstall(
      { mode: "full", features: {} },
      ctx,
    );
    const ponytailPlan = await ponytail.planInstall(
      { mode: "ultra", features: {} },
      ctx,
    );
    ...
    expect(
      (await caveman.install({ ...cavemanPlan, commands: [] }, ctx)).succeeded,
    ).toBe(true);
    expect(
      (await ponytail.install({ ...ponytailPlan, commands: [] }, ctx))
        .succeeded,
    ).toBe(true);
    await expect(
      access(path.join(home, ".claude", ".caveman-active")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
      access(path.join(home, ".config", "ponytail", "config.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  it("uninstall removes only marker-owned files and preserves unrelated config", async () => {
    ...
    await writeFile(
      path.join(home, ".claude", "user-notes.txt"),
      "keep me\n",
      "utf8",
    );
    ...
    await registerHeadroomMcp("codex", headroomMcpSpec("/tmp/headroom"), {
      home,
      platform: "linux",
    });
    ...
    expect(
      await readFile(path.join(home, ".claude", "user-notes.txt"), "utf8"),
    ).toBe("keep me\n");

    const mcp = await unregisterHeadroomMcp("codex", {
      home,
      platform: "linux",
    });
