...
import { readMcpServer, registerHeadroomMcp, headroomMcpSpec } from "../src/mcp.js";
...
describe("mcp registration", () => {
  it("writes codex configuration with comments and markers", async () => {
    ...
    const result = await registerHeadroomMcp("codex", spec, { home, platform: "linux" });
    ...
    const read = await readMcpServer("codex", { home, platform: "linux" }, "headroom");
  ...
  it("refuses to overwrite user-defined codex server without markers", async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), "dont-waste-mcp-"));
    await mkdir(path.join(home, ".codex"), { recursive: true });
    await writeFile(path.join(home, ".codex", "config.toml"), `[mcp_servers.headroom]\ncommand = "/other/headroom"\nargs = ["mcp", "serve"]\n`, "utf8");
    const result = await registerHeadroomMcp("codex", headroomMcpSpec("/tmp/headroom"), { home, platform: "linux" });
    expect(result.status).toBe("mismatch");
    const content = await readFile(path.join(home, ".codex", "config.toml"), "utf8");
  ...
  it("merges Claude and OpenCode JSON without replacing unrelated keys", async () => {
    ...
    await writeFile(path.join(home, ".claude", "mcp.json"), JSON.stringify({ mcpServers: { other: { command: "x" } } }, null, 2), "utf8");
    await writeFile(path.join(home, ".config", "opencode", "opencode.json"), JSON.stringify({ plugin: ["keep"], mcp: {} }, null, 2), "utf8");
    const spec = headroomMcpSpec("/opt/headroom");
    const context = { home, platform: "linux" as const };

    expect((await registerHeadroomMcp("claude-code", spec, context)).status).toBe("registered");
    expect((await registerHeadroomMcp("opencode", spec, context)).status).toBe("registered");

    const claude = JSON.parse(await readFile(path.join(home, ".claude", "mcp.json"), "utf8")) as { mcpServers: Record<string, unknown> };
    expect(claude.mcpServers.other).toEqual({ command: "x" });
    expect(claude.mcpServers.headroom).toEqual({ command: "/opt/headroom", args: ["mcp", "serve"] });

    const opencode = JSON.parse(await readFile(path.join(home, ".config", "opencode", "opencode.json"), "utf8")) as { plugin: string[]; mcp: Record<string, unknown> };
    expect(opencode.plugin).toEqual(["keep"]);
    expect(opencode.mcp.headroom).toMatchObject({ type: "local", command: ["/opt/headroom", "mcp", "serve"], enabled: true });

    expect(await readMcpServer("claude-code", context, "headroom")).toMatchObject(spec);
  });
});
