...
export type MetricType = "input" | "output" | "command-output" | "estimated-output" | "benchmark-reference";
...
export type MetricSummary = {
  ...
  eventCount: number;
  tools: Partial<Record<ToolId, { measuredSaved: number; estimatedSaved: number; events: number }>>;
};
 ...
 */
export function selectNonOverlappingMeasured(events: MetricEvent[]): MetricEvent[] {
  ...
  for (const event of events) {
    ...
    else groups.set(event.overlapKey, [...(groups.get(event.overlapKey) ?? []), event]);
  }
  for (const group of groups.values()) {
    group.sort((left, right) => left.sourceDepth - right.sourceDepth || eventSavings(right) - eventSavings(left));
    const first = group[0];
...
export function aggregateEvents(events: MetricEvent[]): MetricSummary {
  ...
  for (const event of events) {
    ...
    if (selected.some((selectedEvent) => selectedEvent.id === event.id)) current.measuredSaved += eventSavings(event);
