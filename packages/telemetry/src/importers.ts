...
const stableId = (prefix: string, item: unknown): string =>
  `${prefix}-${createHash("sha256").update(JSON.stringify(item)).digest("hex").slice(0, 24)}`;
...
export function sanitizeMetricRecord(
  record: Record<string, unknown>,
): Record<string, unknown> {
  const cleaned: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(record)) {
    if (
      SENSITIVE_METRIC_KEYS.includes(
        key as (typeof SENSITIVE_METRIC_KEYS)[number],
      )
    )
      continue;
  ...
  if (Array.isArray(value))
    return value.filter(
      (item): item is Record<string, unknown> =>
        looseRecord.safeParse(item).success,
    );
...
function attribution(
  record: Record<string, unknown>,
): Pick<MetricEvent, "projectPath" | "agentId" | "sessionId" | "model"> {
  return {
    projectPath:
...
function costs(
  record: Record<string, unknown>,
): Pick<MetricEvent, "costBefore" | "costAfter"> {
  const costBefore = firstNumber(record, [
    "cost_before",
    "costBefore",
    "usd_before",
  ]);
  const costAfter = firstNumber(record, [
    "cost_after",
    "costAfter",
    "usd_after",
  ]);
...
export function importRtkJson(raw: string, now = new Date()): MetricEvent[] {
  ...
  return recordsFrom(parsed).flatMap((rawRecord) => {
    ...
    const before = firstNumber(record, [
      "tokens_before",
      "original_tokens",
      "before_tokens",
      "input_tokens",
    ...
    const after = firstNumber(record, [
      "tokens_after",
      "optimized_tokens",
      "after_tokens",
      "output_tokens",
    ...
    const saved = firstNumber(record, [
      "tokens_saved",
      "saved_tokens",
      "savings",
    ]);
    ...
    const effectiveBefore =
      before ?? (after !== null && saved !== null ? after + saved : null);
    const effectiveAfter =
      after ?? (before !== null && saved !== null ? before - saved : null);
    const command =
    ...
    return [
      {
        ...
        occurredAt: String(
          record.timestamp ?? record.created_at ?? now.toISOString(),
        ),
        tool: "rtk",
        ...
        tokensSaved:
          saved ??
          (effectiveBefore !== null && effectiveAfter !== null
            ? effectiveBefore - effectiveAfter
            : null),
        confidence: "measured",
        sourceDepth: 0,
        overlapKey:
          typeof record.flow_id === "string" ? record.flow_id : undefined,
        ...attrs,
        ...costs(record),
...
export function importHeadroomJson(
  raw: string,
  now = new Date(),
): MetricEvent[] {
  const parsed: unknown = JSON.parse(raw);
  return recordsFrom(parsed).flatMap((rawRecord) => {
    ...
    const output = Boolean(
      record.output ||
      record.metric_type === "output" ||
      record.type === "output-savings",
    );
    const benchmark =
      record.type === "benchmark-reference" ||
      record.metric_type === "benchmark-reference";
    const confidence = benchmark
      ...
      : record.measured === true ||
        ...
        : output
          ? "estimated"
          : "measured";
    const metricType: MetricType = benchmark
      ? "benchmark-reference"
      : output
        ? "estimated-output"
        : "input";
    ...
    return [
      {
        ...
        ...costs(record),
        evidence: benchmark
          ? "headroom benchmark-reference"
          : "headroom perf/output-savings",
      } satisfies MetricEvent,
    ];
  });
}

export function importCavemanStats(
  raw: string,
  now = new Date(),
): MetricEvent[] {
  // Only parse explicit numeric summaries — never treat free-form conversation text as metrics.
  const percent = Number(
    raw.match(/(?:reduction|saved|savings)[^0-9]*(\d+(?:\.\d+)?)\s*%/i)?.[1],
  );
  const saved = Number(
    raw.match(/([\d,.]+)\s*(?:tokens?|tok)\s*saved/i)?.[1]?.replace(/,/g, ""),
  );
  if (!Number.isFinite(percent) && !Number.isFinite(saved)) return [];
  const before =
    Number.isFinite(saved) && Number.isFinite(percent) && percent > 0
      ? saved / (percent / 100)
      : null;
  return [
    {
